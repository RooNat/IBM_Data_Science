...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        data = (filtered_df.groupby('Launch Site')['class'].sum() / filtered_df.groupby('Launch Site')[
        ...
        fig = px.pie(data, values='rate',
                     names=data.index,
                     title='The Rate of the Success and Failure for site {}'.format(entered_site))
        return fig
...
def get_scatter_chart(enteredsite,payload):
    if enteredsite == 'ALL':
        ...
        fig=px.scatter(filtered_df, x='Payload Mass (kg)',y='class',color='Booster Version Category',
                       title="Correlation between Payload and Success for {}".format(enteredsite))
        return fig
