...
app.layout = html.Div([
    ...
    html.H1("Automobile Sales Statistics Dashboard",style={'textAligh':'center','color': '#503D36','font-size':24}),#May include style for title
    ...
    html.Div(id='output-container', className='chart-grid', style={'width':'80%'}),])
])
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        un_vehi=recession_data.groupby('Vehicle_Type')['unemployment_rate'].sum().reset_index()
        R_chart4=dcc.Graph(figure=px.bar(
            un_vehi,
            x='Vehicle_Type',
            y='unemployment_rate',
            title="The effect of unemployment rate on vehicle type"
        ))
...
# Plot 2 Total Monthly Automobile sales using line chart.
        mon=yearly_data.groupby('Month')['Automobile_Sales'].sum().reset_index()
        Y_chart2 = dcc.Graph(
            figure=px.line(
                mon,
                x="Month",
